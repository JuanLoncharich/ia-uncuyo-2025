...
Toda la evidencia numérica proviene de `../vacuum-cleaner-world/experiments_2025-08-25_18-35-26.csv` (560 corridas) y de la versión consolidad en `../vacuum-cleaner-world/reports/tidy_results.csv`. Las visualizaciones fueron generadas automáticamente por `make_reports.py` y se referencian desde `../vacuum-cleaner-world/reports`.
...
- **Métricas registradas:** `performance` (puntaje acumulado), eficiencias de limpieza/movimiento/acción, cantidad de acciones totales y tiempo de simulación. Todas las corridas terminaron con `success=True`, por lo que la comparación se enfoca en eficiencia.

## Resultados cuantitativos
### Promedios globales por agente

| Agente | Corridas | Performance promedio | Eficiencia de limpieza | Eficiencia de movimiento | Eficiencia de acción | Acciones totales | Tiempo de ejecución (s) |
| --- | --- | --- | --- | --- | --- | --- | --- |
| RandomAgent | 280 | 23.24 | 0.031 | 0.047 | 0.186 | 756.23 | 10.74 |
| ReflexiveAgent | 280 | 56.78 | 0.099 | 0.111 | 0.929 | 668.52 | 10.00 |

**Claves:** el ReflexiveAgent logra 2.4× más puntaje y casi 3.2× más eficiencia de limpieza que el RandomAgent, a la vez que necesita 12% menos acciones en promedio. La eficiencia de acción (porcentaje de acciones útiles vs. totales) es prácticamente cinco veces mayor.
...
- ![Distribución del puntaje por agente](../vacuum-cleaner-world/reports/box_perf_by_agent.png) — resume la dispersión del puntaje; los cuartiles del ReflexiveAgent se desplazan por completo sobre los del RandomAgent.
- ![Heatmap RandomAgent](../vacuum-cleaner-world/reports/heatmap_perf_RandomAgent.png) y ![Heatmap ReflexiveAgent](../vacuum-cleaner-world/reports/heatmap_perf_ReflexiveAgent.png) — muestran cómo el puntaje varía según `size` y `dirt_rate`. El mapa del RandomAgent es casi plano más allá de dirt_rate, mientras que el del ReflexiveAgent presenta un gradiente claro al aumentar ambos parámetros.
- ![Performance vs size RandomAgent](../vacuum-cleaner-world/reports/perf_vs_size_RandomAgent.png) y ![Performance vs size ReflexiveAgent](../vacuum-cleaner-world/reports/perf_vs_size_ReflexiveAgent.png) — confirman que el ReflexiveAgent sigue ganando puntaje a medida que el mundo crece, mientras que el RandomAgent se estabiliza rápidamente.
- ![Dispersión performance vs limpieza](../vacuum-cleaner-world/reports/scatter_perf_vs_cleaning_eff.png) — cada punto representa una corrida; la nube del ReflexiveAgent se concentra en el cuadrante superior derecho (puntaje y limpieza altos), evidenciando su consistencia.
...
- No se observaron fallos (`success_rate = 1.0`) en ninguna configuración, lo que indica que la meta de completar el experimento es trivial; el indicador clave debe ser la eficiencia, no el éxito binario.
