...
# Presupuestos razonables (ajustables):
RANDOM_MAX_ITERS = {4: 5_000, 8: 20_000, 10: 40_000, 12: 60_000, 15: 100_000}

SA_PARAMS = lambda n: dict(T_init=n, T_min=1e-3, alpha=0.98, iters_per_T=n*n, max_iters=200_000)
GA_PARAMS = lambda n: dict(population_size=100, generations=1000)
...
def run_random(dim: int, env_n: int) -> RunRecord:
    # FIX: usar string como semilla
    random.seed(f"{dim}-{env_n}-RANDOM")
    board = rnd.Board(dimension=dim)
    max_iters = RANDOM_MAX_ITERS.get(dim, 100_000)
    with FitnessCounter(board) as fc:
...
def run_sa(dim: int, env_n: int) -> RunRecord:
    ...
    with FitnessCounter(board) as fc:
        ...
        elapsed = time.perf_counter() - t0
        best = list(solution)
        best_H = board.fitness(best) if fit is None else fit
    ...
    for size in SIZES:
        for env_n in range(RUNS_PER_SIZE):
            for name, fn in ALGORITHMS:
                print(f"[size={size:>2}] [env={env_n:>2}] [algo={name}] ...", flush=True)
                rec = fn(size, env_n)
                save_path = save_run_json(rec)
                append_csv_row(rec)
                print(f"  -> H={rec.H} | states={rec.states} | time={rec.time:.6f}s | saved: {save_path}", flush=True)
